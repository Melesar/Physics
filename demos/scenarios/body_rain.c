...
void scenario_initialize(program_config* config, physics_config *physics_config) {
  ...
  physics_config->restitution = 0.6;
}
...
void scenario_setup_scene(physics_world *world) {
  ...
  for(count_t i = 0; i < 64; ++i) {
    bool is_box = (i + j) % 2;
    v3 position = { ((int)i - 32) * 5, 5, j * 3 + 2 };

    body b;
    if (is_box) {
      b = physics_add_box_dynamic(world, 10, (v3) { 1, 1, 1 });
    } else {
      b = physics_add_sphere_dynamic(world, 10, 1.0);
    }

    *b.position = position;
    *b.angular_momentum = (v3) { 1, 1, 1 };
